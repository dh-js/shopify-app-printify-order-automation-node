...
function cancellation48Hours_CronJob(db) {
    cron.schedule('05 * * * *', function() {
        console.log(`@@@ Running cron job to cancel orders that have been in the queue over ${process.env.HOW_LONG_WAIT_BEFORE_REFUNDING} hours`);
        db.any(`
            SELECT * FROM orders 
            WHERE 
        ...
        .then(orders => {
            if (orders.length > 0) {
                ...
                console.log(`The following order numbers will be expired: ${orderNumbers.join(', ')}`);
                ...
                sgMail.send(msg).then(() => {
                    console.log(`Email sent: ${orderNumbers.join(', ')} - 48 Hours expired`)
                })
                .catch((error) => {
                    console.error(error)
                });
                ...
                for (const order of orders) {
                    db.none('UPDATE orders SET has_been_cancelled = $1 WHERE id = $2', [`${process.env.HOW_LONG_WAIT_BEFORE_REFUNDING} hours expired - no longer eligible for automation`, order.id])
                        .catch(error => console.error(`Error updating order for automated cancallation ${order.order_number}:`, error));
                }
        ...
        .catch(error => console.error('Error fetching orders for automated cancallation:', error));
    });
}
...
function SyncDBWithShopify_CronJob() {
    cron.schedule('14 * * * *', function() {
      ...
      axios.get(`${process.env.APP_URL}/syncShopifyOrdersToDB`)
        .then(response => console.log('Sync DB to Shopify Cron job completed'))
        .catch(error => console.error('Sync DB to Shopify Cron job failed:', error));
    });
  }
...
function sendOrdersToPrintify_CronJob(db) {
	...
	// 5. required_info_warning must be an empty array
        ...
        db.any(sqlQuery, [process.env.AUTOMATED_ORDER_DELAY, process.env.HOW_LONG_WAIT_BEFORE_REFUNDING])
        .then(orders => {
            if (orders.length > 0) {
                const orderNumbers = orders.map(order => order.order_number);
                console.log(`The following order numbers will be sent to Printify: ${orderNumbers.join(', ')}`);
                ...
                const promises = orders.map(order => {
                    ...
                    if (order.shipping_speed.printify_shipping_method === 'Standard') {
                        shippingMethod = 1;
                    } else if (order.shipping_speed.printify_shipping_method === 'Priority') {
                        shippingMethod = 2;
                    }

                    //Determine items to be ordered
                    let orderedItems = order.ordered_items.filter(item => item.item_is_for_printify === 'Yes').map(({sku, quantity}) => ({sku, quantity}));
                    ...
                    const orderData = {
                        ...
                        "send_shipping_notification": process.env.PRINTIFY_SHIPPING_NOTIFICATIONS === 'on' ? true : false,
                        "address_to": {
                    ...
                    return axios.post(`https://api.printify.com/v1/shops/${process.env.PRINTIFY_SHOP_ID}/orders.json`, orderData, {
                        headers: {
                    ...
                    .then(response => {
                        ...
                        return db.none('UPDATE orders SET order_sent_to_printify = $1 WHERE order_number = $2', [response.data, order.order_number])
                        .catch(error => {
                    ...
                    .catch(error => {
                        ...
                        let hoursDifference = Math.abs(new Date(currentTimeUTC) - new Date(createdAtUTC)) / 36e5;

                        //if it isnt a temporary error then add it to the failed orders array for email notification
                        if (error.response.data.message !== "Action temporarily unavailable, please try again in two hours.") {
                            ordersFailedToSendToPrintify.push(`${order.order_number} - ${error.response.data.errors.reason}`);
                            ordersFailedToSendToPrintifyNumbers.push(order.order_number);

                            return db.none('UPDATE orders SET order_sent_to_printify = $1 WHERE order_number = $2', [error.response.data, order.order_number])
                            .catch(dbError => {
                                console.error(`Failed to update order ${order.order_number} in the database (with it's error object):`, dbError);
                            });
                        ...
                        } else if (error.response.data.message === "Action temporarily unavailable, please try again in two hours." && hoursDifference > (Number(process.env.AUTOMATED_ORDER_DELAY) + 4)) {
                            ordersFailedToSendToPrintify.push(`${order.order_number} - ${error.response.data}`);
                            ordersFailedToSendToPrintifyNumbers.push(order.order_number);

                            return db.none('UPDATE orders SET order_sent_to_printify = $1, has_been_cancelled = $3 WHERE order_number = $2', [error.response.data, order.order_number, "Temp Error - 3 hour aborted"])
                            .catch(dbError => {
                                console.error(`Failed to update order ${order.order_number} in the database (with it's error object):`, dbError);
                            });
                ...
                Promise.allSettled(promises)
                    ...
                    .catch(error => {
                        console.error('Error completing promises when sending orders to Printify:', error);
                    })
                    .finally(() => {
                        if (ordersSuccessfullySentToPrintify.length > 0){
                            console.log(`The following orders were successfully sent to Printify: ${ordersSuccessfullySentToPrintify.join(', ')}`);
                        }
                        if (ordersFailedToSendToPrintify.length > 0) {
                            console.log(`The following orders failed to be sent to Printify: ${ordersFailedToSendToPrintify.join(', ')}`);
                        }
                        console.log(`Cron job to send orders to Printify completed successfully`);

                        if (ordersFailedToSendToPrintify.length > 0) {

                            // Convert the array into a user-friendly string
                            const failedOrdersString = ordersFailedToSendToPrintify.join(', ');
                            const failedOrdersNumbersString = ordersFailedToSendToPrintifyNumbers.join(', ');
                            //Send an email
                            ...
                            sgMail.send(msg).then(() => {
                                console.log(`Email sent - ${failedOrdersNumbersString} Need Manual Fulfillment`)
                            })
                            .catch((error) => {
                                console.error(error)
                            });
        ...
        .catch(error => console.error('Error checking the DB for which orders should be sent to Printify:', error));
...
function deleteOldOrders_CronJob(db) {
    cron.schedule('0 0 * * *', function() {
        ...
        console.log(`@@@ Running cron job to delete orders older than ${days} days`);
        db.none(`DELETE FROM orders WHERE NOW() - created_at > INTERVAL '${days} days'`)
...
function sendPrintifyOrdersProduction_CronJob(db) {
    cron.schedule('32 * * * *', function() {
        ...
        .then(orders => {
            if (orders.length > 0) {
                const promises = orders.map(order => {
                    return axios.get(`https://api.printify.com/v1/shops/${process.env.PRINTIFY_SHOP_ID}/orders/${order.order_sent_to_printify.id}.json`, {
                        headers: {
                            'Authorization': `Bearer ${process.env.PRINTIFY_API_KEY}`
                        }
                    })
                    .then(response => {
                        ...
                        ////////////////////////////////////////////
                        const printifyOrderSKUs = response.data.line_items.map(item => item.metadata.sku);
                        const orderedItemsSKUs = order.ordered_items.filter(item => item.item_is_for_printify === 'Yes').map(item => `${item.sku}${order.printify_sku_region}`);
                        const missingSKUs = orderedItemsSKUs.filter(sku => !printifyOrderSKUs.includes(sku));
                        /////////////////////////////////////////////////////////////////////////////////////////
                        ...
                        if (missingSKUs.length > 0) {
                            console.log(`${order.order_number} Printify order SKUs: `, printifyOrderSKUs);
                            console.log(`${order.order_number} Ordered items SKUs: `, orderedItemsSKUs);
                            console.log(`${order.order_number} Missing SKUs: `, missingSKUs);
                            ...
                            sgMail.send(msg).then(() => {
                                console.log(`${order.order_number} Needs Manual Fulfillment`)
                            })
                            .catch((error) => {
                                console.error(error)
                            });
                            ...
                            return db.none('UPDATE orders SET order_sent_to_printify = jsonb_set(order_sent_to_printify, \'{sent_to_production}\', \'"Error"\') WHERE order_number = $1', [order.order_number])
                            .catch(error => {
                                console.error(`Failed to update order ${order.order_number} sent_to_production value in the database:`, error);
                            });
                            ...
                            return axios.post(`https://api.printify.com/v1/shops/${process.env.PRINTIFY_SHOP_ID}/orders/${order.order_sent_to_printify.id}/send_to_production.json`, {}, {
                                headers: {
                                    'Authorization': `Bearer ${process.env.PRINTIFY_API_KEY}`
                                }
                            })
                            .then(response => {
                                ...
                                if (response.data.hasOwnProperty('id')) {
                                    ...
                                    return db.none('UPDATE orders SET printify_tracking_number = $1, order_sent_to_printify = jsonb_set(order_sent_to_printify, \'{sent_to_production}\', \'"Yes"\') WHERE order_number = $2', [response.data, order.order_number])
                                    .catch(error => {
                                        //log the error message
                                        console.error(`Failed to update order ${order.order_number} in the database message:`, error.message);
                                        console.error(`Failed to update order ${order.order_number} in the database full error:`, error);
                                        
                                    });

                                } else {
                                    errorsArray.push(`${order.order_number}`);
                                    console.log(`Error with response when sending order ${order.order_number} to production - ${response.data}`);
                                }

                            })
                            .catch(error => {
                                console.error(`POST Failed to send order ${order.order_number} to production:`, error.message);
                    ...
                    .catch(error => {
                        console.error(`Error getting created order info from Printify for ${order.order_number}:`, error);
                        errorsArray.push(`${order.order_number}`);
                    });
                });
                ...
                Promise.allSettled(promises)
                    .then((results) => {
                        results.forEach((result, index) => {
                            if (result.status === 'rejected') {
                                console.error(`Promise at index ${index} rejected with ${result.reason}`);
                            }
                        });
                    })
                    .catch(error => {
                    ...
                    .finally(() => {
                        if (ordersSentToProduction.length > 0){
                            console.log(`The following orders are going to be sent to production: ${ordersSentToProduction.join(', ')}`);
                        }

                        if (errorsArray.length > 0) {
                            // Update each order's sent_to_production value to "Error"
                            errorsArray.forEach(orderNumber => {
                                db.none('UPDATE orders SET order_sent_to_printify = jsonb_set(order_sent_to_printify, \'{sent_to_production}\', \'"Error"\') WHERE order_number = $1', [orderNumber])
                                    .catch(error => {
                                        console.error(`Failed to update order ${orderNumber} sent_to_production value in the database:`, error);
                                    });
                            });
                            ...
                            sgMail.send(msg).then(() => {
                                console.log(`Email sent - ${failedOrdersString} Need Manual Fulfillment`)
                            })
                            .catch((error) => {
                                console.error(error)
                            });
    ...
    cron.schedule('41 * * * *', function() {
        ...
        .then(orders => {
            ...
            const promises = orders.map(order => {
                ...
                .catch(error => {
                    console.error(`Error getting tracking info from Printify for ${order.order_number}:`, error);
                });
            });
            ...
            Promise.allSettled(promises)
                    ...
                    .catch(error => {
                        console.error('Error completing promises when getting tracking info from Printify:', error);
                    })
                    .finally(() => {
                        console.log(`Cron job to get tracking info from Printify completed`);
                    });
        })
        .catch(error => {
            console.error('Error getting orders from the database for tracking info:', error);
        });
    });
}
...
function postTrackingInfoToShopify_CronJob(db) {
    cron.schedule('50 * * * *', function() {
        ...
        .then(orders => {
            //log to the console all of the order numbers that are being checked (just needed for testing)
            const orderNumbers = orders.map(order => order.order_number);
            console.log(`The following order numbers will have their tracking info posted to Shopify: ${orderNumbers.join(', ')}`);
            ...
            const promises = orders.map((order, index) => {
                ...
                .then(() => {
                    ...
                    return axios.get(`${process.env.STORE_URL}/admin/api/2023-07/orders/${order.order_id}.json`, {
                        headers: {
                    ...
                    .then(response => {
                        ...
                        if (response.data.order.fulfillments.length > 0 && response.data.order.fulfillments[0].id) {
                            const fulfillmentID = response.data.order.fulfillments[0].id;
                            return axios.post(`${process.env.STORE_URL}/admin/api/2023-07/fulfillments/${fulfillmentID}/update_tracking.json`,
                                {
                                    "fulfillment": {
                                        "notify_customer": true,
                                        "tracking_info": {
                                            "number": order.printify_tracking_number.tracking_number,
                                ...
                                .then(response => {
                                    ...
                                    // Update the printify_tracking_number field in the database
                                    return db.none('UPDATE orders SET printify_tracking_number = jsonb_set(printify_tracking_number, \'{posted_to_shopify}\', \'"Yes"\') WHERE order_number = $1', [order.order_number])
                                    .catch(error => {
                                        console.error(`Failed to update order ${order.order_number} in the database:`, error);
                                    });
                                })
                                .catch(error => {
                                    console.error(`POST Failed to update tracking on Shopify for order ${order.order_number}`, error.message);
                                    //console.error(`POST Failed to send order ${order.order_number} to production:`, error);
                                    errorsArray.push(`${order.order_number}`);
                                    // Update the printify_tracking_number field in the database to Error
                                    return db.none('UPDATE orders SET printify_tracking_number = jsonb_set(printify_tracking_number, \'{posted_to_shopify}\', \'"Error"\') WHERE order_number = $1', [order.order_number])
                                    .catch(error => {
                                        console.error(`Failed to update order ${order.order_number} in the database:`, error);
                                    });
                                });
                            ...
                            console.log(`Order ${order.order_number} has no fulfillment id`);
                    ...
                    .catch(error => {
                        console.error(`Error getting order info from Shopify for ${order.order_number}:`, error.message);
                        errorsArray.push(order.order_number);
                            return db.none('UPDATE orders SET printify_tracking_number = jsonb_set(printify_tracking_number, \'{posted_to_shopify}\', \'"Error"\') WHERE order_number = $1', [order.order_number])
                            .catch(error => {
                                console.error(`Failed to get order ${order.order_number} info from Shopify GET request:`, error);
                            });
                    });
                });
            });
            ...
            Promise.allSettled(promises)
                    ...
                    .catch(error => {
                        console.error('Error completing promises when posting tracking info to Shopify:', error);
                    })
                    .finally(() => {
                        if (noFulfillmentErrorsArray.length > 0){
                            ...
                            sgMail.send(msg).then(() => {
                                console.log(`Email sent - ${failedOrdersString} Need Shopify tracking info updating manually`)
                            })
                            .catch((error) => {
                                console.error(error)
                            });
                        ...
                        if (successfullyUpdatedArray.length > 0) {
                            console.log(`The following orders had their tracking info successfully posted to Shopify: ${successfullyUpdatedArray.join(', ')}`);
                        }
        ...
        .catch(error => {
            console.error('Error getting orders from the database for posting tracking info to Shopify:', error);
        });
